...
static bool externalPWM = false;
static uint64_t pwm_last;
static float network_delta; 
static uint16_t motpow[4];
...
static void stabilizerTask(void* param)
{
	...
	while(1) {
		...
		if (testState != testDone) {
			...
			// Disable actuation with external pwm if it passed  more than 
			// 10 ms since the last message.
			uint64_t time_now = usecTimestamp();
			network_delta = (float)time_now/1000.0f - (float)pwm_last/1000.0f;
			if (externalPWM && (network_delta > 50.0f)) {
				DEBUG_PRINT("Connection Unstable: Disabling external PWM \n");
				externalPWM = false;
			}
					...
					if (controllerType == ControllerTypeDD) {
						if (!DDControllerStarted && setpoint.position.z > 0.01f) {
							...
							if (DDControllerCounter-- <= 0) {
									DEBUG_PRINT("DD Controller Ready\n");
								DDControllerStarted = true;
							}
						} else {
							controllerDD_GetMotorPower(motpow);
...
LOG_GROUP_STOP(health)
	...
	LOG_GROUP_START(networkdt)
	LOG_ADD(LOG_FLOAT, ext_msg_dt, &network_delta)
LOG_GROUP_STOP(networkdt)
